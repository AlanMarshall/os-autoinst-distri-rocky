...
sub run {
    ...
    # open firewall ports
    assert_script_run "firewall-cmd --permanent --add-service=dhcp";
    assert_script_run "firewall-cmd --permanent --add-service=dns";
...
#assert_script_run "printf '<target iqn.2016-06.local.domain:support.target1>\n    backing-store /dev/vdb\n    incominguser test weakpassword\n</target>' > /etc/tgt/conf.d/openqa.conf";
    ...
    # open firewall port
    assert_script_run "firewall-cmd --permanent --add-port=3260/tcp";
    ...
    # configure firewall
    assert_script_run "firewall-cmd --permanent --add-service={nfs,rpc-bind,mountd}";
    assert_script_run "firewall-cmd --permanent --add-port={5555/tcp,5555/udp,6666/tcp,6666/udp}";
    assert_script_run "firewall-cmd --reload";

    # debug output
    assert_script_run "firewall-cmd --list-services";
    assert_script_run "firewall-cmd --list-ports";
